...
@Component({
  tag: 'xmarecek-ambulance-wl-editor',
  styleUrl: 'xmarecek-ambulance-wl-editor.css',
  shadow: true,
})
export class XmarecekAmbulanceWlEditor {
  @Prop() entryId: string;
    ...
    return (
      <Host>
        <form ref={el => this.formElement = el}>
          <md-filled-text-field label="Meno a Priezvisko" required pattern=".*\S.*" value={this.entry?.name}
          ...
          <md-filled-text-field label="Registračné číslo pacienta" required pattern=".*\S.*" value={this.entry?.patientId}
            oninput={(ev: InputEvent) => {
